...
<section id="main" class="wrapper style1">
	<div class="text-center">
		<select class="selectpicker btn-danger" name="heredero" id="heredero">
			<option value=""> &nbsp;&nbsp;&nbsp;Selecciona un Heredero&nbsp;&nbsp;&nbsp;</option>

			@foreach($herederos as $heredero)
			<option style="background: #ec5353; color: #fff;" value="{{$heredero->id_destinatario}}">{{$heredero->nombre}}</option>
			@endforeach
		</select>
		<div id="recuerdo_h">
			<br><br><br><br><br><br>
	...
	function cambiodeh() {
		...
		$.get('/memories/public/api/indexf/' + h_id + '/recuerdos', function(data) {
			...
			if (nombreh != undefined) {
				html_div += '</div></div><br><br><div class="alert alert-danger" role="alert"> Recuerdos de ' + nombreh + '</div><br><br>';
				$('#recuerdo_h').html(html_div);
			} else {
				$('#recuerdo_h').html('</div></div><br><br><div class="alert alert-danger" role="alert"> No hay recuerdos de éste usuario.</div><br><br>');
			}
		});
	}
